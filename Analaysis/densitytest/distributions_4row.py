#!/usr/bin/env python3
"""
4-row × 2-col: distributions of D_f, D_p, n_f^{3D}, φ
...
rho_w, rho_s, mu, g = 1000.0, 2650.0, 1.0e-3, 9.81
...
# Load D_p distributions
dp_samples = {}
for name, sub in [("LOWSHEAR","lowshear"),("MIDSHEAR","midshear"),
                   ("HIGHSHEAR","highshear"),("OM1","om1"),
                   ("OM2","om2"),("OM3","om3")]:
    dp_samples[name] = np.load(f"{RES}/{sub}/{name}_dp_samples.npy") * 1e6  # µm
...
def load(name):
    ...
    Dp_med = np.median(dp_samples[name]) * 1e-6     # m
    phi = (Df_m / Dp_med) ** (nf3d - 3.0)
    ok = (phi > 0) & (phi < 1)
    return {"Df": Df[ok], "nf3d": nf3d[ok], "phi": phi[ok]}
...
fig, axes = plt.subplots(4, 2, figsize=(5.6, 8.6),
...
fig.savefig(f"{OUT}/distributions_4row.png", dpi=300, bbox_inches="tight")
fig.savefig(f"{OUT}/distributions_4row.svg", bbox_inches="tight")
plt.close(fig)
print(f"Saved → {OUT}/distributions_4row.png")
